...
|----------|----------|----------|----------|----------|------|
| 体验营付费线索 Tab | 进入体验营付费线索的学段坐席配置页 | — | Tab 切换按钮 | 点击后切换到体验营付费线索配置内容区；Tab 高亮激活态；与其他 Tab（新媒体998、客服推送、学习机待支付、小程序推送、游学商品、线索品）互斥 | 位于「线索品」Tab 之后，使用 el-tabs card 样式 |

**Tab 样式说明（同现有 Tab 样式）：**

- 默认态：文字色 #303133，背景白色，上/左/右有 1px 边框 `#e4e7ed`
- Hover 态：文字色 #409eff
- 激活态：文字色 #409eff，font-weight: 500，底部边框与白色背景融合（遮盖底部分割线）

### 7.2 体验营付费线索 Tab — 学段切换（Radio 按钮组）

展示形式与「线索品」Tab 完全一致，使用 Radio Button Group 进行学段切换。

| 功能按钮 | 功能说明 | 是否必填 | 功能类型 | 交互说明 | 备注 |
|----------|----------|----------|----------|----------|------|
| 小学 | 切换到小学学段的坐席配置 | — | Radio 按钮 | 点击后高亮该按钮，下方内容区切换为小学学段的配置数据 | 默认选中 |
| 初中 | 切换到初中学段的坐席配置 | — | Radio 按钮 | 点击后高亮该按钮，下方内容区切换为初中学段的配置数据 | — |
| 高中 | 切换到高中学段的坐席配置 | — | Radio 按钮 | 点击后高亮该按钮，下方内容区切换为高中学段的配置数据 | — |

**Radio 按钮样式（同线索品 Tab）：**

- 默认态：背景 `#fff`，文字色 `#606266`，边框 `1px solid #dcdfe6`，padding `8px 15px`
- 激活态：背景 `#409eff`，文字色 `#fff`，边框色 `#409eff`
- 首个按钮左侧圆角 4px，末个按钮右侧圆角 4px

### 7.3 体验营付费线索 Tab — 左右两栏配置区

页面整体为左右两栏布局，比例 1:2（左 33.33%，右 66.67%），与「线索品」Tab 布局一致。

#### 7.3.1 左栏 — 小组选择 + 人员列表

| 功能按钮 | 功能说明 | 是否必填 | 功能类型 | 交互说明 | 备注 |
|----------|----------|----------|----------|----------|------|
| 请选择小组 | 选择组织架构中的小组 | 否 | 级联选择器（Cascader） | 点击展开下拉列表，展示所有小组；选中后在左栏列表区展示该小组下的所有成员 | placeholder: "请选择小组"；样式：el-cascader 组件 |
| 成员列表 | 展示选中小组下的坐席人员列表 | — | 勾选列表 | 每行展示一位成员姓名 + 复选框；勾选后该成员出现在右栏「已选坐席」中；取消勾选则从右栏移除 | 未选择小组时展示空状态"暂无数据" |

**左栏面板样式：**

- 卡片边框：`1px solid #e4e7ed`，圆角 4px
- 头部区域（下拉选择器）：padding `12px 20px`，底部有 `1px solid #e4e7ed` 分割线
- 内容区域（成员列表）：可滚动，固定高度与右栏对齐

#### 7.3.2 右栏 — 已选坐席

| 字段名称 | 字段说明 | 字段来源 | 备注 |
|----------|----------|----------|------|
| 已选坐席（标题） | 右栏顶部标题 | 固定文案 | 样式：padding `18px 20px`，底部分割线 |
| 坐席标签 | 当前学段下已选中的坐席人员 | 左栏勾选操作 + 后端已保存数据 | 以蓝色可关闭标签（Tag）形式展示 |

**坐席标签样式（与线索品 Tab 一致）：**

- 标签容器：padding `20px`，允许换行
- 单个标签：`display: inline-flex`，高度 24px，背景 `#ecf5ff`，文字色 `#409eff`，边框 `1px solid #d9ecff`，圆角 4px，font-size 12px，padding `0 5px 0 9px`，margin `0 10px 10px 0`
- 关闭按钮（×）：14×14px，圆角 50%，色 `#409eff`，hover 时变为白底蓝色
- 无坐席时展示居中占位文案"暂未选择坐席"
...
| 功能按钮 | 功能说明 | 是否必填 | 功能类型 | 交互说明 | 备注 |
|----------|----------|----------|----------|----------|------|
| 重置 | 恢复到上次保存的配置状态 | — | 默认按钮 | 点击后将当前学段的已选坐席恢复为后端上次保存的版本；左栏小组选择清空 | 样式：白底、灰色边框、灰色文字 |
| 确定 | 保存当前学段的坐席配置 | — | 主按钮 | 点击后调用保存接口，将当前学段已选坐席列表提交到后端；成功后 Toast 提示"保存成功" | 样式：蓝底白字 `#409eff` |

**按钮交互规则：**

- 点击「确定」时，如果已选坐席数量为 0，允许保存（清空该学段配置），但弹出二次确认"当前未选择任何坐席，确认保存？"
- 切换学段时，如果当前学段有未保存的修改，Toast 提示"请先保存或重置当前学段配置"（可选，视实际体验决定）
